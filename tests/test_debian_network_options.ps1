...
Assert-Contains $trans 'apt-get -o Acquire::Retries=3 install -y $pkgs' 'The Debian-family custom package installation is missing retry handling.'
Assert-Contains $debianConfig 'for _ in 1 2 3; do' 'The Debian preseed does not retry custom package installation.'
Assert-Contains $debianConfig 'apt-get -o Acquire::Retries=3 update' 'The Debian preseed does not retry refreshing package indexes.'
Assert-Contains $debianConfig 'apt-get -o Acquire::Retries=3 install -y $install' 'The Debian preseed does not install requested packages with retries.'
